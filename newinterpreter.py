...
def subref(x, y, z):
    ...
    def getter():
        return x[y] if isinstance(x, dict) else getattr(x, y)
    def setter(*args):
...
def oper(default, name, rname = ""):
    def inner(*args):
        try:
            return l(default(*args))
        except:
            if len(args) == 1:
                return l(args[0][name]())
            if len(args) == 2:
                try:
                    return l(args[0][name](args[1]))
                except:
                    return l(args[1][rname](args[0]))
    return inner
...
infix_operators = {
    ...
    '||': _(lambda x, y: y % x == 0),
    '%': _(operator.mod),
    '+': _(oper(operator.add, "__add__", "__radd__")),
    '-': _(operator.sub),
    '>': _(operator.gt),
    '<': _(operator.lt),
    '<=': _(operator.le),
...
def get_indices(array, indices):
    if isinstance(array, int):
        ...
        if isinstance(array, str):
            return ''.join(array["__getitem__"](index) for index in indices)
        else:
            return type(array)(array["__getitem__"](index) for index in indices)
...
def stringify(value):
    if isinstance(value, dict):
        if "__str__" in value:
            return value["__str__"]()["default"]
        elif "__repr__" in value:
            return value["__repr__"]()["default"]
        else:
            return repr(value)
    else:
        return repr(value)

def output(*values, end = "\n"):
    print(" ".join(map(stringify, values)), end = end)

def check(string):
    if len(string) == 1: return True
    raise TypeError("ord() expected string of length 1, but " + str(type(string)) + " found")

default["int"] = getfunction(sympy.Integer)
default["float"] = getfunction(sympy.Rational)
default["write"] = getfunction(lambda x: sys.stdout.write(chr(x["default"])) and None or None)
default["flush"] = getfunction(sys.stdout.flush)
default["ord"] = getfunction(lambda x: check(x) and x["default"].bytes)
...
def evaluate(tree, symlist = None, comma_mode = tuple, looped = False, func = False):
    ...
    if 'literal' in tokentype:
        ...
        if 'bracket' in treetype:
            return l(evaluate(tree.children[0], symlist, tuple, looped = looped, func = func))
