...
class Function:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.cache and (args, kwargs) in getkey(self.function):
            ...
            result = l(self.function(*args, **kwargs))
            if self.cache:
...
def l(x):
    ...
    if isinstance(x, (dict, List, type(None))): return x
    res = { k: getattr(x, k) for k in dir(x) if hasattr(x, k) }
    res["default"] = x
    res["class"] = type(x)
    res["__str__"] = lambda: l(str(x))
    res["__repr__"] = lambda: l(repr(x))
    res["__getitem__"] = lambda g: x[g["default"]]
    if isinstance(x, str):
        res["bytes"] = list(map(l, map(sympy.Integer, map(ord, x))))
    return res

def h(x):
    if not isinstance(x, str): return l(x)
    result = ''
    index = 0
    while index < len(x):
        if x[index] == '\\':
